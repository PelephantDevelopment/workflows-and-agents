    ...
    personen = [
        {
            ...
            "foto_url": "https://...",     # optional, URL zum Foto als Fallback wenn Download nicht möglich
            "standort": "Zürich",
...
def _create_person_slide(prs, person, slide_nummer, logo_pfad,
    ...
    if foto_pfad and os.path.exists(foto_pfad):
        ...
        # Fall 2: Nur URL bekannt → Platzhalter mit klickbarem Link
        ...
        # "Foto-Link" Text mit Hyperlink
        txBox = slide.shapes.add_textbox(
            Inches(0.60), Inches(1.45), Inches(2.60), Inches(0.90)
        )
        tf = txBox.text_frame
        tf.word_wrap = True
        p = tf.paragraphs[0]
        p.alignment = PP_ALIGN.CENTER
        _add_text_run(p, "Foto nicht ladbar", FONT_BODY, Pt(11), color=COLOR_WHITE)
        p2 = tf.add_paragraph()
        p2.alignment = PP_ALIGN.CENTER
        p2.space_before = Pt(4)
        run = p2.add_run()
        run.text = "→ Foto öffnen"
        run.font.size = Pt(11)
        run.font.name = FONT_BODY
        run.font.color.rgb = COLOR_LIGHT_GRAY
        run.font.underline = True
        # Hyperlink hinzufügen
        run.hyperlink.address = foto_url
    else:
        # Fall 3: Kein Foto auffindbar → einfacher Platzhalter
        placeholder = slide.shapes.add_shape(
            MSO_SHAPE.OVAL,
            Inches(0.90), Inches(0.90),
            Inches(2.00), Inches(2.00)
    ...
    demo_personen = [
        {
            ...
            "typ": "jurist",
            "foto_pfad": None,
            "foto_url": "https://www.walderwyss.com/team/anna-muster",  # Demo: URL-Fallback
            "standort": "Zürich",
        ...
        },
        {
            ...
            "typ": "unternehmen",
            "foto_pfad": None,
            "foto_url": None,  # Kein Foto gefunden
